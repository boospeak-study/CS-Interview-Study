...
X (TCP는 **연결형(Connection-oriented)** 프로토콜임)
</details>
...
X (서버는 ACK를 먼저 보내고, 남은 데이터를 처리한 후 FIN을 클라이언트에 보냄)
</details>
...
5. 신뢰성이 높아 패킷 손실이 거의 없다. (UDP는 신뢰성이 낮아 패킷 손실이 발생할 수 있음)
</details>
...
5. TCP는 초기 연결 과정 없이 데이터를 바로 전송한다. (TCP는 3-way handshake를 통해 연결을 먼저 설정해야 함)
</details>
